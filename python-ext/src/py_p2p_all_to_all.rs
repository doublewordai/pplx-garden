...
fn low_latency_route_layout_plan_to_dict<'py>(
    ...
    dict.set_item("source_rank_by_final_index", plan.source_rank_by_final_index)?;
...
impl PyAllToAllContext {
    ...
    fn dispatch_send<'py>(
        ...
        use_low_latency_rect_transport: Option<bool>,
        stream: u64,
    ) -> PyResult<Bound<'py, PyDict>> {
        let handle = self
            .ctx
            .dispatch_send(
                ...
                use_low_latency_rect_transport.unwrap_or(false),
                stream,
            )
