...
def expected_canonical_batched_experts_route_plan(
    *,
    ...
    source_group_expert_offsets = [
        [0 for _ in range(num_local_experts)]
        for _ in range(num_source_groups)
    ]
    for route_source_group in source_group_order:
        ...
        for local_expert in range(num_local_experts):
            ...
            source_group_expert_offsets[route_source_group][local_expert] = (
    ...
    return {
        ...
        "source_group_expert_offsets": source_group_expert_offsets,
...
def _row_key(row: torch.Tensor) -> tuple:
    return tuple(row.detach().to("cpu").reshape(-1).tolist())


def assert_batched_experts_layout_semantics(
    *,
    out_expert_x: torch.Tensor,
    out_expert_x_scale: Optional[torch.Tensor],
    expert_num_tokens: torch.Tensor,
    ...
    """Assert BatchedExperts contents without requiring per-expert token order.

    NIXL-EP low-latency assigns per-source expert slots with atomics and carries
    source-token metadata for combine. That shape preserves counts and inverse
    routing without promising stable token order inside each expert. This
    assertion is the order-insensitive gate for that future layout.
    """
    ...
    for local_expert, tokens in enumerate(expected_tokens):
        start = starts[local_expert]
        count = len(tokens)
        actual_tokens = list(flat_out_expert_x[start : start + count])
        assert sorted(map(_row_key, actual_tokens)) == sorted(map(_row_key, tokens))

        if expected_scales is not None:
            assert flat_out_expert_x_scale is not None
            scales = expected_scales[local_expert]
            actual_scales = list(flat_out_expert_x_scale[start : start + count])
